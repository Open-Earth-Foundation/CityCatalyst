...
# GWP values (AR6)
GWP_100YR = {"CO2": 1, "CH4": 29.8, "N2O": 273}

GWP_20YR = {"CO2": 1, "CH4": 82.5, "N2O": 273}


def get_climatetrace_emissions(
    locode: str, year: int, reference_number: str
) -> Optional[Dict]:
    """
...
def parse_city_inventory(
    inventory_df: pd.DataFrame, reference_number: str
) -> Optional[Dict]:
    """
    ...
    try:
        ...
        sector_data = inventory_df[
            inventory_df["GPC Reference Number"] == reference_number
        ]
        ...
        co2_emissions = (
            sector_data["CO2 Emissions"].sum()
            if "CO2 Emissions" in sector_data.columns
            else 0
        )
        ch4_emissions = (
            sector_data["CH4 Emissions"].sum()
            if "CH4 Emissions" in sector_data.columns
            else 0
        )
        n2o_emissions = (
            sector_data["N2O Emissions"].sum()
            if "N2O Emissions" in sector_data.columns
            else 0
        )
        ...
        return {
            ...
            "emissions": emissions,
            "subsector_name": (
                sector_data["Subsector name"].iloc[0]
...
def compare_emissions(
    ...
    if len(sources) < 2:
        ...
        comparison["validation_metrics"][
            "message"
        ] = "Need at least 2 data sources for comparison"
        return comparison
    ...
    if len(emissions_values) >= 2:
        ...
        std_dev = (
            sum((x - mean_emissions) ** 2 for x in emissions_values)
            / len(emissions_values)
        ) ** 0.5
        coefficient_of_variation = (
            (std_dev / mean_emissions * 100) if mean_emissions > 0 else 0
        )
...
def validate_ghg_inventory(
    ...
    for ref_num in reference_numbers:
        ...
        if len(comparison["available_sources"]) >= 2:
            ...
            quality = comparison.get("validation_metrics", {}).get(
                "data_quality_assessment"
            )
            if quality:
