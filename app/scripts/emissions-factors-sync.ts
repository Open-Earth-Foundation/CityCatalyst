...
async function fetchEmissionsFactorData(baseUrl: string) {
  ...
  const endpoints = [
    ...
    {
      path: "/emissions_factor/datasource",
      key: "emissions_factor_datasource",
    },
    {
      path: "/emissions_factor/methodology",
      key: "emissions_factor_methodologies",
    },
    { path: "/emissions_factor/emissions_factor", key: "emissions_factor" },
    {
      path: "/emissions_factor/emissions_factor_datasource",
      key: "emissions_factor_datasource_mapping",
    },
  ];
...
function transformEmissionsFactorData(apiData: Record<string, any>) {
  ...
  const emissionsFactors: EmissionsFactorCreationAttributes[] =
    apiData.emissions_factor.map((ef: EmissionsFactorValue) => ({
      ...
      actorId: ef.actor_id,
      units: ef.units,
      emissionsPerActivity: ef.emissions_per_activity,
      year: ef.year,
      region: ef.region || "global",
      reference: ef.reference,
      methodologyId: ef.methodology_id,
      methodologyName: ef.methodology_name || methodologyMap.get(ef.methodology_id) || null,
      metadata: ef.metadata
        ? typeof ef.metadata === "string"
          ? JSON.parse(ef.metadata)
          : ef.metadata
        : null,
      deprecated: false,
    }));
...
async function markExistingEmissionsFactorsAsDeprecated() {
  ...
  await db.models.EmissionsFactor.update({ deprecated: true }, { where: {} });
}
...
async function syncDataSources(
  dataSources: DataSourceI18nCreationAttributes[],
) {
...
async function syncMethodologies(
  methodologies: MethodologyCreationAttributes[],
) {
  logger.debug("Syncing methodologies...");
  await Promise.all(
    methodologies.map((methodology) =>
      db.models.Methodology.upsert(methodology),
    ),
  );
  console.log(`Synced ${methodologies.length} methodologies`);
}

async function syncEmissionsFactorsData(
  emissionsFactors: EmissionsFactorCreationAttributes[],
) {
  logger.debug("Syncing emissions factors...");
  await Promise.all(
    emissionsFactors.map((emissionsFactor) =>
      db.models.EmissionsFactor.upsert(emissionsFactor),
    ),
  );
  console.log(`Synced ${emissionsFactors.length} emissions factors`);
}

async function syncDataSourceEmissionsFactorRelationships(
  relationships: DataSourceEmissionsFactorCreationAttributes[],
) {
  ...
  console.log(
    `Synced ${relationships.length} datasource-emissionsfactor relationships`,
  );
}
...
async function syncEmissionsFactors() {
  ...
  if (!SKIP_TIMESTAMP_CHECK) {
    console.warn(
      "Timestamp checking not implemented for emissions factors yet, proceeding with sync",
    );
  }
  ...
  try {
    ...
    await syncDataSourceEmissionsFactorRelationships(
      transformedData.dataSourceEmissionsFactors,
    );
