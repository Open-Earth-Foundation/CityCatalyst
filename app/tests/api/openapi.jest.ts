...
describe("OpenAPI Specification Endpoint", () => {
  ...
  describe("GET /api/openapi/json", () => {
    ...
    it("should include core API endpoints", () => {
      ...
      expect(paths).toContain("/api/v0/organizations");
      expect(paths).toContain("/api/v0/user/whoami");
      expect(paths).toContain("/api/v0/check/health");
    });

    it("should have properly structured path definitions", () => {
      const samplePath = "/api/v0/organizations";
    ...
    it("should have consistent API versioning", () => {
      ...
      // Most paths should be under /api/v0/
      const v0Paths = paths.filter(path => path.startsWith("/api/v0/"));
      expect(v0Paths.length).toBeGreaterThan(0);
      
      // Should represent majority of documented endpoints
      expect(v0Paths.length / paths.length).toBeGreaterThan(0.8);
    });
