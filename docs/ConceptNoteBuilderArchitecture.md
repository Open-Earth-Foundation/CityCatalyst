...
is a populated, sanitized example of this exact persisted contract. The
bundle-service test builds it from local GHGI and HIAP capability fixtures
through the production normalization path and asserts exact JSON equality, so
the example fails with the test suite if the runtime contract changes.
