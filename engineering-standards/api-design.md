...
The following patterns in the existing codebase do not conform to these standards. They should be migrated when the relevant endpoints are next modified — not as a dedicated migration exercise.

| Issue | Affected files | Correct pattern |
|---|---|---|
| `v0` and `v1` mixed without rationale | `ghgi_emissions.py`, `population.py` vs all others | Decide canonical version; document the bump rationale |
| Path params used as filters | `ghgi_emissions.py`, `citywide_emission.py`, `region_code.py` | Move `datasource`, `year`, `gpc_ref`, `granularity` to query params |
| Redundant `city` in path | `city_context.py` | `/cities/{locode}/context` |
| Doubled resource name | `/formula_input/formula_input`, `/emissions_factor/emissions_factor` | `/reference/formula-inputs`, `/reference/emission-factors` |
| Uppercase segment in path | `/NO/source/...` in `ghgi_notation_key.py` | `/cities/{locode}/ghgi/notation-key?type=NO` |
| `actor_id` used in path instead of `locode` | `ccra_assessment.py` | Use `locode` as the canonical city identifier |
