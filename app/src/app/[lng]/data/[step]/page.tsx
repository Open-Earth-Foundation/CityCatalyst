...
export default function AddDataSteps({
  ...
  const [disconnedData, setDisconnectData] = useState<boolean>(false);
  ...
  function isSourceDisconnected(source: DataSource): boolean {
    console.log(newlyConnectedDataSourceIds.indexOf(source.datasourceId));
    return (
      (!source.inventoryValues && source.inventoryValues!.length === 0)! ||
      newlyConnectedDataSourceIds.indexOf(source.datasourceId) === -1
    );
