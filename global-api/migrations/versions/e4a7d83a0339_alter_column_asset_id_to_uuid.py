...
def upgrade() -> None:
    ...
    op.add_column("asset", sa.Column("id", sa.dialects.postgresql.UUID, primary_key=True))
    op.create_primary_key('asset_pkey', 'asset', ['id'])

def downgrade() -> None:
    op.drop_constraint('asset_pkey', 'asset', type_='primary')
    op.drop_column("asset", "id")
    op.add_column("asset", sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True))