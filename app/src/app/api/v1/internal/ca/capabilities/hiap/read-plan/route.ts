...
import { readHiapActionPlan } from "@/backend/agentic/hiap/capabilities";
import {
  requireClimateAdvisorServiceRequest,
  requireHiapAgenticEnabled,
  requireRequestUser,
} from "@/backend/agentic/hiap/auth";
import { readHiapPlanInputSchema } from "@/backend/agentic/hiap/registry";
...
export const POST = apiHandler(async (req, { session }) => {
  ...
  const body = readHiapPlanInputSchema.parse(await req.json());
  ...
  const result = await readHiapActionPlan({
    cityId: body.city_id,
