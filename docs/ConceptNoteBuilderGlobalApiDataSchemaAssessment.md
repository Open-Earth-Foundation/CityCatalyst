...
| Field evidence | `funding_record_evidence { claim, quote_or_summary, source_map }` | No opportunity claim-to-source mapping | Missing | Shows where each rule or fact came from; without it reviewers cannot audit template and eligibility claims |
| Funded project | One `funding_records` row with summary, interventions, award, and tags | Rich `/api/v1/projects` records contain summary, financing, interventions, risks, and evidence anchors | Strong schema match; current Chile coverage is narrow | Supplies comparable examples and historical funding behaviour; narrow coverage produces weaker matches |
