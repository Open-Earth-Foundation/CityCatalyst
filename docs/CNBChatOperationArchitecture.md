...
On entry or reload, read current resource state and attach observation if work is still active. The observer must reconcile a saved snapshot when attaching and reconnecting so completion during a disconnect cannot be missed. Close observers on terminal state or workspace disposal. Workspace entry, focus, and reconnect trigger one-off run, draft, and upload revalidation to reconcile changes made elsewhere; this does not promise immediate cross-tab synchronization. Coalesce refreshes for the same resource/version so mutation responses, cache invalidation, and completion events do not cause duplicate reads.
