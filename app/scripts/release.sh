...
function release_test() {
  ...
  git commit -m "chore(release): release candidate version $MAIN_VERSION"
  ...
  git commit -m "chore(release): development version $DEV_VERSION"
  git tag "$DEV_VERSION"
  git push
  git push --tags
...
function release_prod() {
  ...
  git commit -m "chore(release): production version $PROD_VERSION"
