...
# This inspects a populated database; ordinary regression tests use synthetic fixtures.
pytestmark = pytest.mark.skipif(
    not os.getenv("VECTOR_TEST_DATABASE_URL"),
    reason="Set VECTOR_TEST_DATABASE_URL to inspect a database with uploaded embeddings",
)
    ...
    def test_embeddings_table_exists(self):
        ...
        async def _test_table_exists():
            async with self.session_factory() as session:
                ...
                self.assertTrue(
                    result.scalar(), "document_embeddings table should exist"
                )
            ...
            async with self.session_factory() as session:
                ...
                for embedding in embeddings:
                    ...
                    self.assertIsInstance(
                        embedding_id, UUID, "embedding_id should be a UUID"
                    )
                    ...
                    self.assertEqual(
                        ...
                        "model should be text-embedding-3-large",
                    )
                    self.assertIsInstance(
                        chunk_index, int, "chunk_index should be an integer"
                    )
        ...
        async def _test_vector_data():
            async with self.session_factory() as session:
                ...
                self.assertEqual(
                    vector_type,
                    "USER-DEFINED",
                    "Vector column should be USER-DEFINED type",
                )
        ...
        async def _test_metadata():
            async with self.session_factory() as session:
                ...
                for field in required_fields:
                    ...
                    self.assertEqual(
                        null_count, 0, f"No embeddings should have NULL {field}"
                    )
                ...
                self.assertIn(
                    "text-embedding-3-large",
                    models,
                    "Should use text-embedding-3-large model",
                )
    ...
    def test_chunk_continuity(self):
        ...
        async def _test_chunk_continuity():
            async with self.session_factory() as session:
                ...
                print(
                    f"Found {len(chunk_indices)} continuous chunks from 0 to {len(chunk_indices) - 1}"
                )
    ...
    def test_embedding_dimensions(self):
        ...
        async def _test_dimensions():
            async with self.session_factory() as session:
                ...
                print(
                    f"All {total_count} embeddings use text-embedding-3-large model (3072 dimensions)"
                )
