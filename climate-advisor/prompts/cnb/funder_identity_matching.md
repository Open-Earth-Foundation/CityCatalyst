...
- `funded_projects` (array): funded-project records requiring identity review.
  - `funded_project_ref` (string): code-owned project identifier.
...
- `decisions` (array, required): exactly one item for every input funded project.
  - `funded_project_ref` (string, required): copy the input project identifier exactly.
...
<example_output>
{
  "decisions": [
    ...
    {
      "funded_project_ref": "project-002",
      "matches": []
    }
  ]
}
