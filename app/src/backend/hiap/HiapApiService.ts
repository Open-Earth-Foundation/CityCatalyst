...
// Service class for external API calls to the HIAP microservice that need to be mocked in tests
// These are the actual 3rd-party HTTP calls to the HIAP service
export class HiapApiService {
  private static makeHIAPRequest(
  ...
  static async startPrioritization(
    contextData: any,
    type: ACTION_TYPES,
    langs: LANGUAGES[],
    ...
    const response = await this.makeHIAPRequest(
  ...
  /** This Service works with the AI API. In development, run kubectl port-forward svc/hiap-service-dev 8080:80 to access it. */
  static async getPrioritizationResult(
    taskId: string,
  ...
  /** This Service works with the AI API. In development, run kubectl port-forward svc/hiap-service-dev 8080:80 to access it. */
  static async startActionPlanJob({
    action,
    cityId,
    cityLocode,
    lng,
    ...
    try {
      ...
      const startResponse = await this.makeHIAPRequest(
      ...
      while (status === "pending" || status === "running") {
        ...
        const statusResponse = await this.makeHIAPRequest(
      ...
      const planResponse = await this.makeHIAPRequest(
      ...
      try {
        const { actionPlan, created } =
          await ActionPlanService.upsertActionPlan({
            cityId,
            actionId: action.actionId,
   ...
   */
  static async startBulkPrioritization(
    ...
    logger.info(
      ...
      "🔍 City data being sent to HIAP API",
    );
   ...
   */
  static async checkBulkPrioritizationProgress(
    taskId: string,
  ): Promise<{ status: string; error?: string }> {
    const url = `/prioritizer/v1/check_prioritization_progress/${taskId}`;
    ...
    const json = await response.json();
    logger.info(
      "checkBulkPrioritizationProgress response received successfully",
    );
   ...
   */
  static async getBulkPrioritizationResult(
    taskId: string,
  ...
  static async translateActionPlan(
    ...
    while (status === "pending" || status === "running") {
      const statusResp = await this.makeHIAPRequest(
    ...
    const planResp = await this.makeHIAPRequest(
      `/plan-creator/v1/get_plan/${taskId}`,
    );
