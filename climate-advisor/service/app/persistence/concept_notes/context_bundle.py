...
class ContextBundleRepositoryError(Exception):
    """Repository failure with a stable public code and HTTP status."""
...
class ContextBundleRepository:
    """Store build progress and guard bundle commits by active build ID."""
        ...
        *,
        ...
        try:
            async with self._session_factory() as session, session.begin():
                ...
                if progress.get("build_id") != str(build_id) or not await (
                    _matches_current_source_fingerprint(
                        session=session,
                        run_id=run_id,
                        progress=progress,
                    )
                ...
                selected_upload_ids = {
                    source.upload_id for source in selected_sources
                }
                if (
...
def get_context_bundle_repository() -> ContextBundleRepository:
    """Provide the configured context-bundle repository."""
    return ContextBundleRepository(get_session_factory())
