"""Synchronous client for upstream action mitigation feasibility scores."""
...
DEFAULT_ACTION_MITIGATION_FEASIBILITY_SCORES_BASE_URL = (
...
def get_action_mitigation_feasibility_scores_base_url() -> str:
    """Return the configured shared Global API host."""
    raw_value = os.getenv("CCGLOBAL_API_BASE_URL")
    if raw_value is None or not raw_value.strip():
        return DEFAULT_ACTION_MITIGATION_FEASIBILITY_SCORES_BASE_URL
    return raw_value.strip()


@dataclass
class ActionMitigationFeasibilityScoresApiService:
    """Fetch and map city-scoped mitigation feasibility scores."""

    base_url: str | None = None

    def __post_init__(self) -> None:
        """Resolve the upstream feasibility scores host from config when omitted."""
        if self.base_url is None:
            self.base_url = get_action_mitigation_feasibility_scores_base_url()

    def _build_action_mitigation_feasibility_scores_url(
        self, locode: str, country_code: str
    ) -> str:
        """Return the full upstream feasibility scores URL for one city."""
        ...
        return (
            ...
            f"action-mitigation-feasibility-scores?{query}"
        )
    ...
    def get_scores_by_action_id(
        ...
        try:
            ...
            if error.status_code != 404:
                raise
            warning = (
                "action mitigation feasibility scores API returned 404; continuing "
        ...
        try:
            response = ActionMitigationFeasibilityScoresApiResponse.model_validate(
                payload
            )
            ...
            raise UpstreamApiError(
                status_code=502,
                message=(
                    "action mitigation feasibility scores API returned a payload "
        ...
        for score in response.scores:
            ...
            if action_id in scores_by_action_id:
                raise UpstreamApiError(
                    status_code=502,
                    message=(
                        "action mitigation feasibility scores API returned duplicate "
                        f"src_action_id values for locode={locode.strip().upper()}"
                    ),
                    upstream_status_code=http_status_code,
                    url=scores_url,
                )
            ...
            scores_by_action_id[action_id] = (
                ActionMitigationFeasibilityScoreRecord.model_validate(
                    {
                        "action_id": action_id,
                        "locode": score.locode,
