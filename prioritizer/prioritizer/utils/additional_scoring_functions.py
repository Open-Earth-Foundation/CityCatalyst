def count_matching_hazards(city, action):
    ...
    city_hazards = {
        entry["hazard"]
        for entry in city.get("ccra", [])
        if entry["normalised_risk_score"] and entry["normalised_risk_score"] > 0.5
    }

    # Get the action's hazards
    action_hazards = (
        set(action.get("Hazard", [])) if action.get("Hazard") is not None else set()
    )
    ...
    matching_percentage = (
        len(matching_hazards) / total_hazards if total_hazards > 0 else 0
    )
