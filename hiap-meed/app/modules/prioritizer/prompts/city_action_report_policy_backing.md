...
- `facts.policy_score` (object or null): selected-action policy score, aggregate finding/document counts, selected evidence rows, omitted-incomplete count, selection note, and support category when available
...
Do not quote, name, or cite policy text unless it appears in `facts.policy_score.policy_evidence`. Do not reconstruct omitted or incomplete excerpts. If a row has no page number, use a natural target-language equivalent of "not available" rather than inventing a page. If policy evidence is missing, state that the report cannot confirm policy backing from the available information.
</output>
