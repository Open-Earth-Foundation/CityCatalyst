/**
 * @swagger
 * /api/v1/chat/threads/{inventory}:
 *   post:
 *     tags:
 *       - Chat
 *     summary: Create a new chat thread for inventory via Climate Advisor
 ...
 *           type: string
 *           format: uuid
 *         description: Inventory UUID for chat context
 *     requestBody:
 *       required: false
 *       content:
 *         application/json:
...
async function issueCaUserToken(params: { 
  ...
  const response = await fetch(`${process.env.CC_BASE_URL}/api/v1/internal/ca/user-token`, {
    method: 'POST',
    headers: {
