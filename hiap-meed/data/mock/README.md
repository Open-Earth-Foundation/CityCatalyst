...
The required `meta.requestId` is used only for caller correlation. All
other metadata is generated by HIAP-MEED in the response.
...
The request does not repeat the route, timestamp, provider, or record count.

# prioritizer_bulk_request_mock.json:

This is the multi-city variant of the same frontend request contract. It uses
the same schema as `prioritizer_request_mock.json` but includes
