...
fuel_to_gpc = {
    ...
    'Liquefied Petroleum Gases (LPG)': ['II.1.1', 'II.2.1', 'II.3.1', 'II.4.1', 'II.5.1'], 
    ...
    'Petrol (100% mineral petrol) (Motor Gasoline)': ['II.1.1', 'II.2.1', 'II.3.1', 'II.4.1', 'II.5.1'],
...
transport_type_from_source_to_cc = {
    ...
    'Agriculture Equipment': {
        'II.5.1': ['Agricultural machinery']
    }, 
    'Forestry Equipment': {
        'II.5.1': ['Forestry equipment']
    },
    'Industry Equipment': {
    ...
    'Construction Equipment2': {
        'II.5.1': ['Construction maquinery']
    }, 
    'Lawn and Garden Equipment': {
        'II.5.1': ['Household equipment']
    },
    'Airport Equipment': {
        'II.5.1': ['Airport equipment']
    }, 
    ...
    'Freight flights': {
        'II.4.1': ['Cargo aircraft']
    }, 
    'Vans': {
        'II.1.1': ['Passenger vehicles', 'Commercial vehicles', 'Service vehicles', 'Emergency vehicles']
    }, 
    'HGV - Rigid': {
    ...
    'Sea tanker': {
        'II.3.1': ['Marine vessels', 'Ferries']
    },
    'Cargo ship': {
        'II.3.1': ['Marine vessels']
    },
    'Medium-Duty Truck5': {
        'II.1.1': ['Commercial vehicles'],
        'II.5.1': ['Mining equipments', 'Construction maquinery']
    ...
    'Bus': {
        'II.1.1': ['Public transport vehicles'],
        'II.5.1': ['Airport equipment']
    },
    'Medium-Duty Truck5': {
        'II.1.1': ['Commercial vehicles'],
    ...
    'Air - Short Haul1, up to 3700km distance': {
        'II.4.1': ['all']
    },
    'Air - Long Haul1, over 3700km distance': {
        'II.4.1': ['all']
    },
    'Air - International1': {
        'II.4.1': ['all']
...
if __name__ == "__main__":
    ...
    # raw data file path
    input_fl = "../data_raw/ghgprotocol/Emission_Factors_for_Cross_Sector_Tools_V2.0_0.xlsx"
    ...
    # Drop empty columns
    df1_co2 = df1_co2.drop(columns=['Unnamed: 0', 'Unnamed: 6', 'Unnamed: 7', 'Unnamed: 8'])

    # Restructure
    df1_co2 = df1_co2.melt(id_vars=['Region', 'Fuel', 'EF Unit'], 
    ...
    # rename columns
    df1_co2.columns = ['actor_id', 'fuel_type', 'units', 'gas', 'emissions_per_activity']

    # Rename gases
    df1_co2['gas'] = df1_co2['gas'].replace({
        'Fossil CO2 EF': 'CO2',
        'Biogenic CO2 EF': 'biogenic CO2'
    })
    ...
    # Rename columns
    df1_nonCO2.columns = ['NA','actor_id','fuel_type','subcategory_type', 'enginee_type','CH4_EF','CH4_units','N2O_EF','units']
    ...
    df1_nonCO2 = df1_nonCO2.melt(id_vars=['actor_id', 'fuel_type', 'subcategory_type', 'enginee_type','units'], 
                        value_vars=['CH4_EF', 'N2O_EF'], 
                        var_name='gas', 
                        value_name='emissions_per_activity')

    # Rename gases
    df1_nonCO2['gas'] = df1_nonCO2['gas'].replace({
        'CH4_EF': 'CH4',
        'N2O_EF': 'N2O'
    })

    # assign "GPC_refno" using the mapping dic
    df1_nonCO2['gpc_reference_number'] = df1_nonCO2['subcategory_type'].map(transport_type_to_gpc)
    ...
    for index, row in df1_nonCO2.iterrows():
        ...
        if subcategory_type in transport_type_from_source_to_cc:
            if gpc_reference_number in transport_type_from_source_to_cc[subcategory_type]:
                df1_nonCO2.at[index, 'transport_type'] = transport_type_from_source_to_cc[subcategory_type][gpc_reference_number]
    ...
    tmp3 = conversions(df1, 'g/US Gallon', 'kg/US Gallon', 1e-3, 'emissions_per_activity * g:kg')

    # Concatenate all the dfs
    df1 = pd.concat([df1, tmp1, tmp2, tmp3], ignore_index=True)

    # convert US Gallon to L and m3
    tmp4 = conversions(df1, 'kg/US Gallon', 'kg/L', 1/3.785, 'emissions_per_activity * US Gallon:L')
    tmp5 = conversions(df1, 'kg/US Gallon', 'kg/m3', 1/0.004, 'emissions_per_activity * US Gallon:m3')

    # convert scf to L and m3
    tmp6 = conversions(df1, 'kg/scf', 'kg/L', 1/28.317, 'emissions_per_activity * scf:L')
    tmp7 = conversions(df1, 'kg/scf', 'kg/m3', 1/0.028, 'emissions_per_activity * scf:m3')
    ...
    df1.loc[df1['units'] == 'kg/TJ', 'NCV'] = df1.loc[df1['units'] == 'kg/TJ', 'fuel_type'].map(conversion_df['NCV'])
    df1.loc[df1['units'] == 'kg/TJ', 'density'] = df1.loc[df1['units'] == 'kg/TJ', 'fuel_type'].map(conversion_df['density'])
    ...
    # rename columns
    df_1.columns = ['actor_id', 'subcategory_type', 'subcategory_type2', 'weight', 'fuel_type', 'emissions_per_activity', 'units']
    ...
    # rename columns
    df_2.columns = ['actor_id', 'subcategory_type', 'subcategory_type2', 'weight', 'fuel_type', 'CH4', 'units', 'N2O', 'units2']

    # change the format
    df_2 = pd.melt(df_2,
                id_vars=['actor_id', 'subcategory_type', 'subcategory_type2', 'weight', 'fuel_type', 'units'],
                value_vars=['CH4', 'N2O'],
                var_name='gas',
                value_name='emissions_per_activity')
    ...
    df2[['subcategory_type', 'subcategory_type2', 'weight', 'fuel_type']] = df2[['subcategory_type', 'subcategory_type2', 'weight', 'fuel_type']].fillna('all')

    # apply unit conversions
    tmp10 = conversions(df2, 'g/tonne-kilometer', 'kg/ton-km', 1e-3, 'emissions_per_activity * g:kg')
    tmp11 = conversions(df2, 'g/short ton-mile', 'kg/short ton-mile', 1e-3, 'emissions_per_activity * g:kg')
    tmp12 = conversions(df2, 'g/vehicle-mile', 'kg/vehicle-mile', 1e-3, 'emissions_per_activity * g:kg')

    tmp13 = conversions(df2, 'kg/short ton-mile', 'kg/kg-km', (1/907.19)*(1/1.609), 'emissions_per_activity * short ton:kg and mile:km')
    tmp14 = conversions(df2, 'kg/vehicle-mile', 'kg/km', 1/1.609, 'emissions_per_activity * mile:km')

    tmp15 = conversions(df2, 'g/tonne-kilometer', 'kg/kg-km', 1, 'emissions_per_activity * g:kg and tonne:kg')
    tmp16 = conversions(df2, 'g/short ton-mile', 'kg/kg-km', 1e-3*(1/907.19)*(1/1.609), 'emissions_per_activity * g:kg, short tone to kg and mile to km')
    tmp17 = conversions(df2, 'g/vehicle-mile', 'kg/km', 1e-3*(1/1.609), 'emissions_per_activity * g:kg and mile:km')

    # Concatenate all the dfs
    df2 = pd.concat([tmp10, tmp11, tmp12, tmp13, tmp14, tmp15, tmp16, tmp17], ignore_index=True)
    ...
    #rename columns
    df3.columns = ['actor_id', 'subcategory_type', 'subcategory_type2', 'CO2', 'CO2_units', 'CH4', 'CH4_units', 'N2O', 'N2O_units']

    #change format
    df3 = pd.melt(df3,
            id_vars=['actor_id', 'subcategory_type', 'subcategory_type2', 'CO2_units', 'CH4_units', 'N2O_units'],
    ...
    # apply unit conversions
    tmp10 = conversions(df3, 'kg/passenger-mile', 'kg/passenger-km', (1/1.609), 'emissions_per_activity * mile:km')
    tmp12 = conversions(df3, 'kg/vehicle-mile', 'kg/vehicle-km', (1/1.609), 'emissions_per_activity * mile:km')
    tmp13 = conversions(df3, 'g/passenger-kilometer', 'kg/passenger-kilometer', 1e-3, 'emissions_per_activity * g:kg')
    tmp14 = conversions(df3, 'g/passenger-mile', 'kg/passenger-km', 1e-3, 'emissions_per_activity * g:kg')
    tmp15 = conversions(df3, 'g/vehicle-mile', 'kg/vehicle-km', 1e-3, 'emissions_per_activity * g:kg')
    tmp16 = conversions(df3, 'g/passenger-mile', 'kg/passenger-km', 1e-3*(1/1.609), 'emissions_per_activity * g:kg and mile:km')
    tmp17 = conversions(df3, 'g/vehicle-mile', 'kg/vehicle-km', 1e-3*(1/1.609), 'emissions_per_activity * g:kg and mile:km')

    # Concatenate all the dfs
    df3 = pd.concat([tmp10, tmp11, tmp12, tmp13, tmp14, tmp15, tmp16, tmp17], ignore_index=True)
    ...
    df3[['transport_type', 'subcategory_type', 'weight', 'fuel_type']] = df3[['transport_type', 'subcategory_type', 'weight', 'fuel_type']].fillna('all')
