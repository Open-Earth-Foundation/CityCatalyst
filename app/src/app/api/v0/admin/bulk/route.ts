...
const createBulkInventoriesRequest = z.object({
  ...
  emails: z.array(z.string()), // Comma separated list of emails to invite to the all of the created inventories
  years: z.array(z.number().int()), // List of years to create inventories for (can be comma separated input, multiple select dropdown etc., so multiple years can be chosen)
